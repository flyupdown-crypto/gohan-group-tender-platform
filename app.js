...
const moduleDefinitions = [
  ...
  { code: "M03", name: "ERP / WMS 深度能力", description: "验证供应商是否具备真正 ERP / WMS 深度，而非轻量库存或普通 POS 能力。", defaultCategory: "C", items: ["多仓体系", "中央仓与门店仓", "库区管理", "库位管理", "货架管理", "批次管理", "保质期管理", "FIFO / FEFO 出库规则", "入库差异单", "出库流水", "入库流水", "库存变更日志", "库存损耗记录", "多仓调拨", "多仓拆单", "合并订单出库", "部分出库", "自动补货建议", "安全库存规则", "最低库存规则", "采购审批流", "出库审批流", "财务审批流", "客户授信额度", "欠款控制", "应收款管理", "应付款管理", "多税率配置", "打印模板自定义"] },
  { code: "M04", name: "供应链与采购库存管理", description: "验证集团采购、供应商、库存、追溯和成本管理的业务闭环。", defaultCategory: "B", items: [["总部采购管理", "B"], ["供应商评级", "B"], ["成本分析", "B"]] },
  { code: "M05", name: "多系统集成与硬件适配", description: "验证 Open API、Webhook、OAuth、安全合规和关键硬件能力。", defaultCategory: "F", items: [["Open API", "F"], ["Webhook", "F"], ["第三方 API 集成", "F"], ["OAuth / SSO", "F"], ["数据加密", "F"], ["权限隔离", "F"], ["多语言切换", "F"], ["多时区切换", "F"], ["系统消息提醒", "F"], ["高并发能力", "F"], ["数据备份", "F"], ["灾备恢复", "F"], ["KDS 设备适配", "H"], ["扫码设备适配", "H"], ["标签打印机适配", "H"], ["设备远程诊断", "H"]] },
  ...
  { code: "M08", name: "报价、隐性成本与风险披露", description: "验证供应商是否完整披露培训、运维、多仓部署和隐性成本。", defaultCategory: "E", items: ["培训费用披露", "运维费用披露", "多仓部署费用披露", "隐性成本说明"] },
];
...
function renderSupplierIntro() {
  return `<div class="background-block supplier-intro"><h3>GOHAN GROUP 项目说明</h3><p>本项目为 Gohan Group 跨业态数字化系统招标，覆盖餐饮、零售、娱乐及未来酒店业务。供应商需逐项说明功能支持情况、功能深度、标准或定制、预计工时、费用和风险说明；Open API、第三方依赖和案例统一在供应商基础能力区填写一次。</p></div>`;
}
